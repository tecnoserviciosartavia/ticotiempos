...
class CalculateWinners implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($idventa_cabecera)
    {
    ...
    public function handle()
    {
        ...
        if (count($jugadas_ganadoras) > 0) {
            ...
            foreach ($jugadas_ganadoras as $gano) {
                ...
                Venta_detalle::where('id',$gano->id)->update(['estatus' => 'ganadora', 'es_ganador' => 1, 'monto_ganador' => $monto_ganador]);
                ...
                Transacciones::create([
                    'idusuario' => $gano->idusuario,
                    'monto' => $monto_ganador,
                    'concepto' => 'Jugada Ganadora del ticket '.$identificador.' por el monto de '.$monto_ganador,
                    'tipo_concepto' => 'premio',
                    'json_dinamico' => json_encode($callback),
                ]);
                $user_balance = New User_balance;
                $user_balance->actualizarSaldoConPremio($gano->idusuario, $monto_ganador);
            }
        }
        ...
        if (count($jugadas_perdedoras) > 0) {
            ...
            foreach ($jugadas_perdedoras as $perdio) {
                ...
                //Actualizo el estatus a Calculada correctamente y mantengo los demas parametros
                Venta_detalle::where('id',$perdio->id)->update(['estatus' => 'calculada']);
            }
        }

        Venta_cabecera::where('id',$this->venta_cabecera)->update(['estatus' => 'finalizado']);
